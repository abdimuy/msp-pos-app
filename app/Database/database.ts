...
export const replaceTableLocal = async (
  tableName: string,
  newSchema: string,
  txn?: Tx
): Promise<void> => {
  ...
  await database.runAsync(`ALTER TABLE nueva_${tableName} RENAME TO ${tableName};`);
};
...
export const updateSavedSchemaLocal = async (tableName: string, txn?: Tx): Promise<void> => {
  ...
  const schemaData = await database.getFirstAsync<{ sql: string }>(
  ...
  await database.runAsync(`DELETE FROM esquemas_guardados WHERE nombre_tabla = ?;`, [tableName]);

  // Inserta el nuevo registro
  await database.runAsync(`INSERT INTO esquemas_guardados (nombre_tabla, esquema) VALUES (?, ?);`, [
    tableName,
    schemaData.sql,
  ]);
};
...
export const compareAndModifyLocal = async (txn?: Tx): Promise<void> => {
  ...
  const currentSchemas = await getTableSchemasLocal(txn); // Recupera las tablas actuales de todas las tablas

  // Recorre cada tabla obtenida
  for (const { name, sql: currentSchema } of currentSchemas) {
    // Consulta la última versión guardada del esquema
    const previousSchema = await database.getFirstAsync<{ schema: string }>(
    ...
    if (previousSchema && previousSchema.schema !== currentSchema) {
      console.log(`El esquema de la tabla ${name} ha cambiado.`);

      // Reemplaza la tabla con el nuevo esquema
      await replaceTableLocal(name, currentSchema, txn);
