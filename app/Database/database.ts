...
let dbVentas: SQLite.SQLiteDatabase;
...
export const initSalesDB = async () => {
  try {
    dbVentas = await SQLite.openDatabaseAsync('ventas.db');
    await dbVentas.execAsync(`
      PRAGMA journal_mode = WAL;
    ...
    console.error('Error al inicializar la base de datos de ventas:', error);
  }
};
...
export const insertarVenta = async (venta: Sale): Promise<void> => {
  try {
    ...
    const result = await dbVentas.runAsync(
...
export const obtenerVentas = async (): Promise<Sale[]> => {
  try {
    const ventasBase = await dbVentas.getAllAsync<any>(`SELECT * FROM sale;`);
    for (const venta of ventasBase) {
      const imagenes = await dbVentas.getAllAsync<{ url: string }>(
...
export const eliminarVenta = async (saleId: number): Promise<void> => {
  try {
    await dbVentas.runAsync(`DELETE FROM sale WHERE id = ?;`, [saleId]);
